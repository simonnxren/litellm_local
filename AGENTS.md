...
LiteLLM Gateway (:8400)  — Unified API, caching, retries, logging
    │
   ├── model="local_vlm"             → Chat Service       (:8070)  Qwen3-VL-4B-Instruct-FP8   12 GB
   ├── /api/ocr/chat/completions     → OCR VLM Server     (:8118)  PaddleOCR-VL-1.5-0.9B       4-5 GB
   ├── /api/ocr/*                    → OCR API Service    (:8080)  PaddleOCR layout pipeline    4-5 GB
   ├── /api/embeddings               → Embedding Service  (:8090)  Qwen3-VL-Embedding-2B-FP8   4 GB
   └── /api/asr/transcriptions       → ASR Service        (:8060)  Qwen3-ASR-1.7B               6 GB
```
...
| OCR (OpenAI-compatible pass-through) | `PaddleOCR-VL-1.5-0.9B` | ~0.9B | 8118 | paddleocr-genai-vllm-server | 4-5 GB |
| OCR (doc pipeline) | `PaddleOCR-VL-1.5` pipeline | — | 8080 | paddleocr-vl offline API + paddlex | 4-5 GB |
| ASR (speech→text) | `Qwen/Qwen3-ASR-1.7B` | 1.7B | 8060 | vLLM nightly (cu130) | 6 GB |
| Embeddings (multimodal) | `alexliap/Qwen3-VL-Embedding-2B-FP8-DYNAMIC` | 2B | 8090 | vLLM nightly (cu130) + pooling runner | 4 GB |
...
| `pipeline_config_vllm.yaml` | PaddleOCR API pipeline config (`paddlex --serve`) for layout parsing and markdown restructuring. |
| `vllm_config.yaml` | Backend config passed to `paddleocr genai_server`. |
...
| 8080 | OCR API | PaddleOCR-VL pipeline endpoints | ✅ Operational |
| 8118 | OCR VLM | PaddleOCR-VL-1.5-0.9B | ✅ Operational |
| 8090 | Embedding | Qwen3-VL-Embedding-2B-FP8 | ✅ Operational (2048 dims) |
| 8060 | ASR | Qwen3-ASR-1.7B | ✅ Operational |
...
- **OCR VLM Service**: `gpu-memory-utilization` in `vllm_config.yaml` (~4.8-6.4 GB depending on tuning)
...
- ✅ **Pass-through-first Inference Routing**: OCR VLM and ASR endpoints are exposed via pass-through to preserve backend request/response formats
- ✅ **Paddle OCR Gateway Pass-through**: Added `/api/ocr/layout-parsing` and `/api/ocr/restructure-pages` through LiteLLM
- ✅ **OCR Split Architecture**: OCR VLM runs on `:8118`, while doc parsing API runs on `:8080`
...
5. **Multimodal Embeddings via Gateway**: LiteLLM's built-in `/v1/embeddings` proxy only forwards the `input` field (standard OpenAI format). vLLM multimodal embeddings require a `messages` field (`EmbeddingChatRequest` format) which LiteLLM strips. The built-in `/vllm/*` pass-through was also tested but malforms URLs (double `/v1/v1/` prefix).
   - **Fix**: Use pass-through endpoints only for embeddings: `/api/embeddings` and `/api/embeddings/models`.
   - **Result**: The Python SDK (`litellm_client.py`) uses `/api/embeddings` exclusively for both text and multimodal embeddings.
